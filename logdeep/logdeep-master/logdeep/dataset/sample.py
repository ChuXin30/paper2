...
def sliding_window(data_dir, dir2 ,  datatype, window_size, sample_ratio=1):
    '''
    dataset structure
        result_logs(dict):
        ...
        labels(list)
    '''
    event_output_file = data_dir + 'hdfs2/event_vector_top3.txt'
    ...
    labels = []
    if datatype == 'train':
        data_dir += dir2+ '/train_normal'
    if datatype == 'val':
        data_dir += dir2+ '/test_normal'

    with open(data_dir, 'r') as f:
        for line in f.readlines():
            num_sessions += 1
            #line = tuple(map(lambda n: n - 1, map(int, line.strip().split())))
            line = tuple(map(lambda n: n, map(int, line.strip().split()))) #bgl

            for i in range(len(line) - window_size):
                Sequential_pattern = list(line[i:i + window_size])
                #Quantitative_pattern = [0] * 28
                #Quantitative_pattern = [0] *378
                Quantitative_pattern = [0] * 1916 #loganomaly_bgl_drain_random_with_level_20
                ...
                #         Semantic_pattern.append(event2semantic_vec[str(event)]) #bgl
              #  print(len(Semantic_pattern))
              #  print((len(Semantic_pattern[0])))
              #  print((np.array(Semantic_pattern)).shape)

                Sequential_pattern = np.array(Sequential_pattern)[:,
                                                                  np.newaxis]
                # Quantitative_pattern = np.array(
                #     Quantitative_pattern)[:, np.newaxis]
                #print(Quantitative_pattern.shape)#(28, 1)
                ...
                #print(result_logs['Quantitatives'].shape)
...
def session_window(data_dir, datatype, sample_ratio=1):
    #event2semantic_vec = read_json(data_dir + 'hdfs/event2semantic_vec.json')
    event_output_file = data_dir + 'hdfs/event_vector_top3.txt'
    event2semantic_vec = KeyedVectors.load_word2vec_format(event_output_file, binary=False)
    result_logs = {}
    result_logs['Sequentials'] = []
    result_logs['Quantitatives'] = []
