...
class loganomaly(nn.Module):
    ...
    def forward(self, features, device):
       ...
       # print(input0.shape) #orch.Size([2048, 10, 1])

        #print(input1.shape)#torch.Size([2048, 28, 1])
        #print(input1.size(0))#2048

        h0_0 = torch.zeros(self.num_layers, input0.size(0),
                           self.hidden_size).to(device)
        #print(h0_0.shape)#torch.Size([2, 2048, 64])
        c0_0 = torch.zeros(self.num_layers, input0.size(0),
                           self.hidden_size).to(device)
        #print(h0_0.shape)#torch.Size([2, 2048, 64])
        out0, _ = self.lstm0(input0, (h0_0, c0_0))
        #print(out0.shape)#torch.Size([2048, 10, 64])
        ...
        #print(multi_out.shape)#torch.Size([2048, 128])
        out = self.fc(multi_out)#torch.Size([2048, 28, 64])
        #print(out1.shape)
        return out
