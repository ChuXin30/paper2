...
#            line = list(map(lambda n: n , map(int, line.strip().split())))
            line = line + [0] * (10 + 1 - len(line))
           # line = line + [-1] * (window_size + 1 - len(line))
            # hdfs.add(tuple(line))
            hdfs[tuple(line)] = hdfs.get(tuple(line), 0) +1 
            # hdfs.append(tuple(line))
    print('Number of sessions({}): {}'.format(name, len(hdfs)))
    return hdfs , len(hdfs)
...
#            ln = list(map(lambda n: n - 1, map(int, ln.strip().split())))
            ln = list(map(lambda n: n, map(int, ln.strip().split()))) #bgl

            ln = ln + [-1] * (window_size + 1 - len(ln))
            hdfs[tuple(ln)] = hdfs.get(tuple(ln), 0) + 1
            length += 1
    print('Number of sessions({}): {} {}'.format(name, len(hdfs),str(length)))
    return hdfs, length
...
class Predicter():
    ...
    def predict_unsupervised(self):
        ...
        # test_normal_loader, test_normal_length = generate('hdfs_test_normal_simple')
        # test_abnormal_loader, test_abnormal_length = generate('hdfs_test_abnormal_simple')
        ...
        test_normal_loader, test_normal_length = generate_hdfs( self.data_dir+ self.dir2 + '/test_normal' )
        test_abnormal_loader, test_abnormal_length = generate_hdfs( self.data_dir+ self.dir2 + '/abnormal' )
        TP = 0
        FP = 0
        # Test the model
...
#                    seq1 = [0] * 28
                    ...
                    seq1 = torch.tensor(seq1, dtype=torch.float).view(
                        -1, self.num_classes+1, self.input_size).to(self.device)
